...
let handler = async (m, {
    ...
    if (!lister.includes(feature)) return m.reply("هذا الامر خاص بتحميل برامج الحاسوب من الواتساب فقط من موقع \nhttps://filehippo.com\nيمكنك تحميل البرامج من خلال كتابة \n*.exe s+facebook*\nعندما نحصل على رابط البرنامج نأتي الان ونكتب للبوت هكذا حتى يقوم بتحميله \n*.exe d+(رابط البرنامج)*\n\n" + lister.map((v, index) => "  ○ " + v).join("\n"))
...
📃 *summary:* ${item.summary}`
        ...
        if (feature == "d") {
            if (!inputs) return m.reply("مثال:\n.exe d+https://filehippo.com/download_facebook-windows-10/")
            await m.reply(wait)
            try {
